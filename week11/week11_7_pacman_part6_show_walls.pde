...
void draw(){
  ...
  if (x+dx >= 05 && x+dx <= 600-15 && canMoveX) x += dx; //改變座標 (不超過4周的牆)
  if (y+dy >= 05 && y+dy <= 600-15 && canMoveY) y += dy; //改變座標 (不超過4周的牆)
  //豆子 和 轉折點 座標(15 + j*30, 15 + i*30)
  if( (x-15)%30 == 0 && (y-15)%30 == 0){
    if(next==2) {dx = -1; dy = 0; dir = 2; } //dir:0 向右0度
    if(next==0) {dx = +1; dy = 0; dir = 0;} //dir:2 向左PI
