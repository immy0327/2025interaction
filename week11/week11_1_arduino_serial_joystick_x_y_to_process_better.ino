//week11_1_arduino_serial_joystick_x_y_to_process_better
...
void loop() {
  ...
  else x = (x-x0)/4.4 + 128; //有大的值,就減掉中間的值,再餘4,再加128
  if ( abs(y-y0) < 25 ) y = 128; //數值變化太小,直接放中間值128
  else y = (y-y0)/4.4 + 128; //有大的值,就減掉中間的值,再餘,再加128
  Serial.write(x); //直接算出去
  Serial.write(y); //直接算出去
